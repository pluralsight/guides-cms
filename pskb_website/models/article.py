...
def save_article(title, path, message, new_content, author_name, email, sha,
    ...
    # metadata for branched articles should be identical to the original
    # article. This makes it easier for automatically merging changes because
    # metadata differences won't get in the way.  The author_name is the only
    # thing useful for a branched article.  However, that should already be
    # encoded in the branch name and the commits.  So, editors of original
    # articles will get credit for helping via those mechanisms, not metadata.
    if branch != 'master':
...
def branch_article(article, message, new_content, author_name, email):
    ...
    orig = models.article.read_article(article.path)
    branch = author_name

    # Create branch if we needed to
    repo_sha, status = remote.read_branch(orig.repo_path, branch)
    if status == 404:
        repo_sha, status = remote.read_branch(orig.repo_path, 'master')
        assert repo_sha is not None, 'Cannot find master branch'

        if not remote.create_branch(orig.repo_path, branch, repo_sha):
            # FIXME: Handle error
            return None

    return models.article.save_article(orig.title, orig.path, message,
                                       new_content, author_name, email,
                                       orig.sha, branch=branch)
...
def save_branched_article_meta_data(article, author_name, email):
    ...
    Note the process of saving the metadata for a branched article includes
    saving the metadata for the original article in order to properly track the
    branch names.
    """
    ...
    if article.branch not in orig_article.branches:
        orig_article.branches.append(article.branch)
        status = save_article_meta_data(orig_article, author_name, email)
        if status != 200:
            return status

    # Now save identical metadata to the branch for this article
